...
&nbsp;&nbsp;&nbsp;&nbsp;
A interface do **RStudio** é dividida em quatro painéis principais (veja mais em [painéis do RStudio](https://docs.posit.co/ide/user/ide/guide/ui/ui-panes.html)), cada um desempenhando um papel distinto para melhorar a produtividade e eficiência nas suas tarefas de programação em **R**:
...
&nbsp;&nbsp;&nbsp;&nbsp;
Vamos ter o nosso primeiro contato com o R. A partir de agora, tudo que estiver nesses quadro coloridos, você deve copiar (pode usar o botão no canto superior direito) e colar no seu _R script_.
...
>> DICA: Quando uma linhas que começa com **#** tem **----** no final, ela vira uma seção, que pode ser buscada no índice de seções do código.
...
### Utilizando objetos em operações lógicas
...
Agora você tem um código que pega dois números quaisquer e te dá a multiplicação deles.
...
Veja só! Você calculou diversas multiplicações com a mesma faciliade que você fez o cálculo com apenas dois números!
...
Funções são ferramentas que recebem um objeto ( _input_ ) de entrada, processam sua informação e devolvem um resultado ( _output_ ). Também podem receber *argumentos* que complementam as instruções para o funcionamento da função.
...
tabela_exp <- data.frame(antes = c(130, 100, 90,150,130,120),
...
O [**ggplot2**](https://ggplot2-book.org/) é um pacote do R amplamente utilizado para visualização de dados, criado por [**Hadley Wickham**](https://hadley.nz/) e lançado em 2005 como parte de seu trabalho para o RStudio e a comunidade R, é baseado na gramática dos gráficos, que permite construir gráficos de forma estruturada, consistente e altamente personalizável. 
