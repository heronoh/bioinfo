...
Nesta prática iremos explorar um subconjunto de dados do sistema SIM-DO, mais especificamente analisando óbitos relacionados à.
...
```{r import_data_susrados, echo=TRUE,eval=FALSE}
...
Para facilitar, iremos trabalhar com um subconjunto já organizado, que deve ser baixado [aqui](https://drive.google.com/file/d/1JKU12N-x_8_PcdRJHX6MzRtrZZDRqvET/view?usp=sharing). Você irá fazer download do arquivo e em seguida iremos carregar esse arquivo para o R. Para as análises da desta aula prática, você pode pular a próxima sessão.
...
dados_SUS$IDADEanos %>% table()
dados_SUS$SEXO %>% table()


# visualizar quantas observações temos de cada tipo
dados_SUS$IDADEanos %>% table() %>% plot()
dados_SUS$SEXO %>% table() %>% plot()
...
```{r graficos, echo=TRUE,eval=FALSE}
